  ...
  public void close() {
    for (SocketEntry entry : sockets.values()) {
      ...
      TransportStateEvent e =
              new TransportStateEvent(TLSTM.this, entry.getPeerAddress(),
...
/*
   ...
   *    an SSL engine.
   * @return
   *    {@code true} if inbound and outbound is done.
   */
...
//      sslEngineConfigurator.configure(SSLContext.getDefault(), useClientMode);
    ...
    public void closeSession() {
      ...
      try {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug("Channel "+this+ " already closed while sending CLOSING message");
        }
      }
      catch (IOException e) {
        logger.error("IOException while closing outbound channel of " + this + ": " + e, e);
      }
    }
  }
  ...
  class ServerThread implements WorkerTask {
    ...
    private final LinkedList<SocketEntry> pending = new LinkedList<SocketEntry>();
          ...
          try {
            ...
            synchronized (entry.inboundLock) {
              ...
              if (entry.getInNetBuffer().position() > 0) {
                ...
                if (isConnectionClosed(entry, null)) {
                  continue;
                }
                entry.addRegistration(selector, SelectionKey.OP_READ);
              }
    ...
    private boolean isConnectionClosed(SocketEntry entry, IOException ioException) {
      if (isEngineClosed(entry.sslEngine)) {
        ...
        closeChannel(entry.socket.getChannel());
        removeSocketEntry(entry.getPeerAddress());
    ...
    private void processPending() {
      synchronized (pending) {
        for (int i=0; i<pending.size(); i++) {
          ...
          if (!entry.hasMessage()) {
            if (logger.isDebugEnabled()) {
              logger.debug("Idle socket entry removed from pending message queue: " + entry);
            }
    ...
    public boolean runDelegatedTasks(SSLEngineResult result, SocketEntry entry) throws IOException {
      ...
      switch (result.getStatus()) {
        case BUFFER_UNDERFLOW:
          ...
          return false;
        case CLOSED:
          entry.removeRegistration(selector, SelectionKey.OP_READ | SelectionKey.OP_WRITE);
      ...
      switch (status) {
        ...
        case NOT_HANDSHAKING:
          ...
          return true;
        default:
          logger.debug("Unrecognized TLS result: "+status);
      }
      return false;
    }

    protected void connectSocketToSendMessage(Address address, byte[] message,
                                              SocketChannel socketChannel, SocketEntry entry,
                                              Map<Address, SocketEntry> sockets) {
      SocketEntry prevSocketEntry = sockets.putIfAbsent(address, entry);
      if (prevSocketEntry != null) {
        if (prevSocketEntry.getSocket().isConnected()) {
          ...
          // use last connection
          entry.insertMessages(prevSocketEntry.message);
    ...
    private void closeRedundantSocketChannelIfNeeded(Address address,
                                                     SocketChannel socketChannel,
                                                     SocketEntry prevSocketEntry, boolean closeNew) {
      if (socketChannel != prevSocketEntry.getSocket().getChannel()) {
        ...
        try {
          if (closeNew) {
            ...
            prevSocketEntry.getSocket().close();
          }
        } catch (IOException iox) {
        ...
        do {
          ...
          if (result.getStatus() == SSLEngineResult.Status.OK) {
            if (result.bytesProduced() > 0) {
              ...
              if (logger.isDebugEnabled()) {
                logger.debug("SSL Engine status of wrap is ok, but 0 NET bytes produced, handshake status is "+
