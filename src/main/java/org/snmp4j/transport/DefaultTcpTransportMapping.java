...
public class DefaultTcpTransportMapping extends TcpTransportMapping {
  ...
  class SocketEntry {
    ...
    private AtomicInteger busyLoops = new AtomicInteger(0);
    ...
    public int nextBusyLoop() {
      return busyLoops.getAndIncrement();
    }
  ...
  protected class ServerThread implements WorkerTask {
    ...
    protected synchronized void connectSocketToSendMessage(Address address, byte[] message,
                                                           SocketChannel socketChannel, SocketEntry entry,
                                                           Map<Address, SocketEntry> sockets) throws ClosedChannelException {
      ...
      if (prevSocketEntry != null) {
        if (prevSocketEntry.getSocket().isConnected()) {
          ...
          if (logger.isDebugEnabled()) {
            logger.debug("Concurrent connection attempt detected, closing recent to " + address);
          }
          entry.addMessage(message);
          closeRedundantSocketChannelIfNeeded(address, socketChannel, prevSocketEntry, true);
        } else {
          if (logger.isDebugEnabled()) {
            logger.debug("Concurrent connection attempt detected, closing previous to " + address);
          }
          // use last connection
    ...
    private void queueNewMessage(SocketEntry entry) throws ClosedChannelException {
      synchronized (pending) {
        pending.add(entry);
      }
      selector.wakeup();
