...
public class Snmp implements Session, CommandResponder {
    ...
    private final Map<Address, OctetString> contextEngineIDs =
            Collections.synchronizedMap(new WeakHashMap<Address, OctetString>());
    ...
    private synchronized void ensurePendingTimer() {
        if (timer == null) {
     ...
     *
     * @return the cached context engine ID retrieved by RFC 5343 discovery or {@code null} if no such ID had been
     * cached yet or the cached value was already removed either explicitly with {@link
     * #removeCachedContextEngineId(Address)} or implicitly because the address object was no longer associated with any
     * open connection (i.e., the internally {@link WeakHashMap} recognized no more active references on the address
     * key).
     * @since 2.8.10
     */
     ...
     *         the target address that returned the context engine ID.
     *
     * @return the removed cached context engine ID retrieved by RFC 5343 discovery or {@code null} if no such ID had
     * been cached yet.
     * @since 2.8.10
     */
